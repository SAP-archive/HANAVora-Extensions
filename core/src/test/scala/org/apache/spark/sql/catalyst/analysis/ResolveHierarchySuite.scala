...
class ResolveHierarchySuite extends FunSuite with MockitoSugar {
  ...
  test("Check parenthood expression has no conflicting expression IDs and qualifiers") {
    ...
    /* Currently, we keep the same exprId, this might change in the future */
    assert(parenthoodExpression.left.asInstanceOf[Attribute].exprId ==
      parenthoodExpression.right.asInstanceOf[Attribute].exprId)
  }
