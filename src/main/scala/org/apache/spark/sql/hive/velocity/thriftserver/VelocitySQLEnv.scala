...
object VelocitySQLEnv extends Logging {

  def init() {
    logDebug("Initializing VelocitySQLEnv")
    if (hiveContext == null) {
      logInfo("Creating VelocitySQLContext")
      ...
      hiveContext = new VelocitySQLContext(sparkContext)
